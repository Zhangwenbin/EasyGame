...
public abstract class CanvasWindow : UIWindow
{
		...
		set
		{
			if (_canvas != null)
			{
				...
				for (int i = 0; i < _childCanvas.Length; i++)
				{
					...
					{
						//保留原来的层级差
						canvas.sortingOrder = depth + _childCanvas[i].sortingOrder - _canvas.sortingOrder;
					}
				}
	...
	/// 监听按钮点击事件
	/// </summary>
	protected Button AddButtonListener(string path, UnityEngine.Events.UnityAction call)
	{
