...
namespace Farmhash.Sharp
{
    ...
    public static class Farmhash
    {
        ...
        /// <returns>A 32bit hash</returns>
        public static unsafe uint Hash32(char[] chars, int length)
        {
            if (chars == null) throw new System.ArgumentNullException(nameof(chars));
            if ((uint)length > (uint)chars.Length) throw new System.ArgumentOutOfRangeException(nameof(length));

            fixed (char* buf = chars)
            {
                return Hash32((byte*)buf, length * sizeof(char));
            }
        }
        ...
        /// <param name="s">char[] to calculate the hash on</param>
        /// <returns>A 32bit hash</returns>
        public static unsafe uint Hash32(char[] chars, int offset, int length)
        {
            if (chars == null) throw new System.ArgumentNullException(nameof(chars));
            if ((uint)length > (uint)chars.Length) throw new System.ArgumentOutOfRangeException(nameof(length));
            if ((uint)offset > (uint)chars.Length) throw new System.ArgumentOutOfRangeException(nameof(offset));
            if (length + offset > chars.Length) throw new System.ArgumentOutOfRangeException(nameof(length));

            fixed (char* buf = &chars[offset])
            {
                return Hash32((byte*)buf, length * sizeof(char));
            }
        }
