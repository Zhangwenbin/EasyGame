...
namespace FMODUnity
{
    ...
    public class StudioEventEmitter : EventHandler
    {
        ...
        public static bool bankLoaderReady = false;
        ...
        public void Play()
        {
            ...
            if ( FMODUnity.Settings.Instance.ImportType==ImportType.AssetBundle)
            {
                if (!bankLoaderReady)
                {
                    return;
                }
                bool loaded = FMODUnity.RuntimeManager.HasBankLoaded(bank);
                if (!loaded)
                {
                    var handle= Addressables.LoadAssetAsync<TextAsset>(bank+".bytes");
